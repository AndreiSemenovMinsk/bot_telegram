 ...
 */
@Component
public class WithdrawPartnerEnd4 implements Command {
    ...
    public LevelResponse runCommand(Update update, Level level, User users) throws CloneNotSupportedException {
        ...
        System.out.println();
        System.out.println("*****************WithdrawPartnerEnd4****************");
        System.out.println();
        ...
        try {
            ...
            partner.setLim(new BigDecimal(sum));
            partnerCacheRepository.save(partner);


            Shop shopDebtor = shopCacheRepository.findById(partner.getDebtor());
            Shop shopCreditor = shopCacheRepository.findById(partner.getCreditor());
            ...
            Message message = new Message(null, Map.of(LanguageEnum.RU,"Вы установили размер начисления кэшбека " + partner.getRate()
                    + "% и лимит " + partner.getLim() + " для " + shopDebtor.getName()));
            ...
            Message message2 = new Message(null, Map.of(LanguageEnum.RU,"На Вас установили размер начисления кэшбека " + partner.getRate()
                    + "% и лимит " + partner.getLim() + " для " + shopCreditor.getName()));
