...
@AllArgsConstructor
@NoArgsConstructor
public class P2BWriteOffCoupon implements Command {
    ...
    public LevelResponse runCommand(Update update, Level level, User users) throws IOException, WriterException {
        ...
        System.out.println();
        System.out.println("+++++++++++++++++++++P2BWriteOffCoupon++++++++++++++++++++");
        System.out.println();
        System.out.println();
        ...
        try {
            ...
            List<Action> actionCouponList = actionRepository
                    .findAllByShopAndTypeAndActive(shopInitiator.getId(), ActionTypeEnum.COUPON, true);

            Action actionCouponDefault = actionRepository
                    .findFirstByShopAndTypeAndActive(shopInitiator.getId(), ActionTypeEnum.COUPON_DEFAULT, true);
            actionCouponList.add(actionCouponDefault);

            for (Action action : actionCouponList) {

                ButtonRow row = new ButtonRow();
                Button approveButton = new Button(row, Map.of(users.getLanguage(), action.getName()), initialLevel.level_P2B_WRITEOFF_COUPON_REQUEST.getIdString() + action.getId());
